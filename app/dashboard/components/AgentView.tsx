...
export default function AgentView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  ...
  async function fetchDailyData() {
    ...
    else setPerfInput({ call: 0, meet: 0, pt: 0, intro: 0, db_assigned: 0, db_returned: 0, contract_cnt: 0, contract_amt: 0, target_cnt: 10, target_amt: 300, edu_done: false });
  }
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <button onClick={() => setIsToolOpen(true)} className="bg-black text-[#d4af37] border-2 border-black py-4 rounded-2xl font-black text-[11px] uppercase shadow-lg hover:scale-105 transition-all">Sales Tools</button>
      </div>
      ...
      <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border">
        ...
        <button onClick={handleSave} className="w-full mt-8 bg-black text-white py-6 rounded-[2rem] font-black uppercase tracking-widest shadow-2xl transition-all active:scale-95">Save Daily Data</button>
      </div>
...
function MetricInput({ label, val, onChange, color }: any) {
  return (
    <div className="space-y-2 text-center font-black">
      <label className="text-[10px] text-slate-300 uppercase">{label}</label>
      <input type="number" value={val || ''} onChange={e=>onChange(Number(e.target.value))} className={`w-full p-4 bg-slate-50 border-2 border-transparent focus:border-black rounded-2xl text-center text-xl outline-none ${color}`} />
    </div>
  )
}
  ...
  return (
    <div className="bg-white p-8 rounded-[3rem] border shadow-sm space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <input type="number" value={tar} onChange={e=>onChangeTar(Number(e.target.value))} className="p-4 bg-slate-50 rounded-2xl font-black text-center outline-none" />
        <input type="number" value={cur} onChange={e=>onChangeCur(Number(e.target.value))} className={`p-4 bg-slate-50 rounded-2xl font-black text-center outline-none ${color}`} />
      </div>
      <div className="space-y-2">
        <div className="flex justify-between items-end font-black px-2"><span className="text-[9px] text-slate-400 uppercase">{unit} 달성률</span><span className="text-3xl italic">{pct.toFixed(1)}%</span></div>
        <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${color.replace('text-', 'bg-')} transition-all duration-700`} style={{ width: `${pct}%` }} /></div>
      </div>
    </div>
  )
}