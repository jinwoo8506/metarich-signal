...
export default function AgentView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  ...
  async function fetchHistoryData() {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      dates.push(`${prev.getFullYear()}-${String(prev.getMonth() + 1).padStart(2, '0')}-01`);
    }
  ...
  const avgData = useMemo(() => {
    const count = historyData.length || 1;
    const sum = historyData.reduce((acc, curr) => ({
      ...
      db: acc.db + (curr.db_assigned || 0)
    }), { amt: 0, cnt: 0, call: 0, meet: 0, pt: 0, intro: 0, db: 0 });
          ...
          <div className="bg-slate-900 p-8 rounded-[3rem] text-white font-black shadow-xl">
            ...
            {avgTab === 'perf' ? (
              ...
              <div className="grid grid-cols-4 gap-2 font-black">
                ...
                <AvgBox label="소개/배정" val={`${avgData.intro}/${avgData.db}`} />
              </div>
            )}
          </div>
