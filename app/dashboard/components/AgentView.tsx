...
export default function AgentView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  // [신규] 메인 탭 상태: 'input' (실적/활동), 'edu' (교육확인)
  ...
  const toggleEdu = () => {
    ...
    handleSave({ edu_status: nextStatus });
  };
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom-4 pb-20 font-black">
      ...
      <div className="flex gap-2 font-black">
        <button 
          ...
          className={`flex-1 py-4 rounded-2xl border-2 border-black italic transition-all ${mainTab === 'input' ? 'bg-black text-[#d4af37]' : 'bg-white text-black opacity-40'}`}
        >
          PERFORMANCE
        </button>
        <button 
          onClick={() => setMainTab('edu')}
          className={`flex-1 py-4 rounded-2xl border-2 border-black italic transition-all ${mainTab === 'edu' ? 'bg-black text-[#d4af37]' : 'bg-white text-black opacity-40'}`}
        >
          EDUCATION
        </button>
      </div>
