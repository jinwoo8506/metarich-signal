...
export default function LeaderView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  ...
  const [branchStats, setBranchStats] = useState<any[]>([]); // 지점별 합산 데이터
  ...
  // 사업부 전체 활동 합산 데이터
  const [totalActivity, setTotalActivity] = useState({ call: 0, meet: 0, pt: 0, intro: 0, amt: 0, cnt: 0 });
  ...
  async function fetchDepartmentData() {
    ...
    // 2. 사업부 소속 인원만 필터링 (leader는 본인의 department_name 기준)
  ...
  return (
    <div className="flex-1 space-y-6 font-black p-4 md:p-6 text-black">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 bg-slate-50 p-4 rounded-[2rem] border-2 border-black">
        ...
        <button onClick={() => setActiveTab('finance')} className="bg-white border-2 border-black p-4 rounded-2xl text-xs italic hover:bg-black hover:text-[#d4af37] transition-all font-black uppercase">영업도구(계산기)</button>
        <button onClick={() => setShowExportOpt(!showExportOpt)} className="bg-black text-[#d4af37] p-4 rounded-2xl text-xs italic font-black uppercase relative">
          ...
          {showExportOpt && (
            <div className="absolute top-full left-0 mt-2 bg-white border-2 border-black rounded-xl shadow-2xl z-50 w-full text-black">
              <button onClick={() => handleExport('excel')} className="w-full p-3 border-b hover:bg-slate-100">EXCEL</button>
              <button onClick={() => handleExport('pdf')} className="w-full p-3 hover:bg-slate-100">PDF</button>
            </div>
          )}
        </button>
          ...
          {branchStats.map(branch => (
            <div key={branch.name} className="p-5 bg-slate-900 text-white rounded-[1.5rem] border-2 border-black">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-[10px] opacity-60">{branch.members}명 활동 중</span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[9px] uppercase opacity-50">Branch AMT</p>
                  <p className="text-xl italic">{branch.amt.toLocaleString()}만</p>
                </div>
                <div>
                  <p className="text-[9px] uppercase opacity-50">Branch CNT</p>
                  <p className="text-xl italic">{branch.cnt}건</p>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* 5. 개인별 세부 모니터링 */}
      <section className="bg-white p-6 rounded-[2.5rem] border-2 border-slate-100 shadow-sm">
        <h2 className="text-lg mb-6 border-l-8 border-slate-300 pl-4 italic uppercase font-black">Individual Monitoring</h2>
        <div className="space-y-4">
          {agents.map(a => {
            const amtRate = Math.round(((Number(a.performance.contract_amt) || 0) / (Number(a.performance.target_amt) || 1)) * 100);
            return (
              <div key={a.id} onClick={() => { setSelectedAgent(a); setActiveTab('act'); }} className="p-5 bg-slate-50 rounded-[1.5rem] border-2 border-transparent hover:border-black cursor-pointer transition-all">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    <span className="text-[10px] text-slate-400 ml-2">{a.branch_name}</span>
                  </div>
                ...
                <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                  <div className="bg-blue-500 h-full transition-all" style={{ width: `${Math.min(amtRate, 100)}%` }} />
                </div>
              </div>
            );
          })}
      ...
      {activeTab && !['finance'].includes(activeTab) && (
        <AdminPopups
          ...
          onClose={() => { setActiveTab(null); setSelectedAgent(null); fetchDepartmentData(); }}
        />
      )}
    </div>
  )
