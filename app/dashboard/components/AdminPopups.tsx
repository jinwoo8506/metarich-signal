...
export default function AdminPopups({ type, agents, teamMeta, onClose }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-xl flex items-center justify-center p-6 font-black font-black">
      <div className="bg-white w-full max-w-5xl rounded-[4rem] p-12 relative overflow-y-auto max-h-[90vh] shadow-2xl font-black font-black">
        <button onClick={onClose} className="absolute top-10 right-10 text-3xl font-black hover:rotate-90 transition-all font-black font-black">✕</button>
        ...
        {type === 'act' && (
          <div className="space-y-8 animate-in slide-in-from-top-4 font-black">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-black font-black">
          ...
          <div className="space-y-10 font-black">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 font-black">
              <div className="space-y-6 font-black">
                <InputItem label="팀 목표 금액 (만)" val={tarAmt} onChange={setTarAmt} />
                <InputItem label="팀 목표 건수 (건)" val={tarCnt} onChange={setTarCnt} />
                <InputItem label="도입 목표 (명)" val={tarIntro} onChange={setTarIntro} />
                <InputItem label="도입 실제 결과" val={curIntro} onChange={setCurIntro} />
              </div>
              <div className="space-y-6 font-black">
...
function StatBox({ label, cur, tar, unit, color }: any) {
  ...
  return (
    <div className="bg-slate-50 p-8 rounded-[3.5rem] border text-center font-black transition-all hover:bg-white hover:shadow-xl font-black">
      <p className="text-[10px] text-slate-400 mb-2 uppercase tracking-widest font-black font-black">{label}</p>
