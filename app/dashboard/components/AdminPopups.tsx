  ...
  return (
    <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 font-black">
      ...
      <div className="bg-white w-full max-w-5xl rounded-[2rem] md:rounded-[4rem] p-6 md:p-12 relative overflow-y-auto max-h-[90vh] border-4 border-black shadow-2xl">
        ...
        {type === 'sys' && (
          <button onClick={saveSys} className="absolute top-6 right-16 md:top-8 md:right-20 bg-black text-[#d4af37] px-4 py-2 rounded-full text-xs italic font-black uppercase hover:bg-slate-800 transition-all z-50">
            Save Configuration
          </button>
        )}
            ...
            <div className="border-2 md:border-4 border-black rounded-2xl md:rounded-[2.5rem] overflow-hidden">
              <table className="w-full text-left font-black">
                <thead className="bg-black text-[#d4af37] text-[10px] uppercase">
                  <tr><th className="p-3 md:p-6">Employee Name</th><th className="p-3 md:p-6 text-center">Amount</th><th className="p-3 md:p-6 text-center">Status</th></tr>
                </thead>
                  ...
                  {agents.map((a: any) => {
                    ...
                    return (
                      <tr key={a.id} className={`hover:bg-slate-50 transition-colors ${blinkClass}`}>
                        <td className="p-3 md:p-6 font-black text-sm md:text-xl italic">{a.name} CA</td>
                        <td className="p-3 md:p-6 text-center font-black text-sm md:text-2xl italic">{a.performance.contract_amt}만</td>
                        <td className="p-3 md:p-6 text-center">
                          <button onClick={() => handleApprove(a.id, a.performance.is_approved)} className={`px-4 py-1 md:px-6 md:py-2 rounded-full text-[10px] md:text-xs font-black italic border-2 border-black transition-all ${a.performance.is_approved ? 'bg-black text-[#d4af37]' : 'bg-white text-black hover:bg-slate-100'}`}>
            ...
            {!selectedAgent ? (
              <div className="space-y-6 md:space-y-12 animate-in fade-in duration-500">
                <div className="bg-slate-50 p-6 md:p-10 rounded-2xl md:rounded-[3rem] border-2 md:border-4 border-black grid grid-cols-3 gap-2 md:gap-6 text-center shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] md:shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
                   <div><p className="text-[10px] text-slate-400 uppercase mb-2">총 배정 DB</p><p className="text-lg md:text-3xl font-black italic">{totalDB}건</p></div>
                   <div><p className="text-[10px] text-slate-400 uppercase mb-2">총 반품 DB</p><p className="text-lg md:text-3xl text-rose-500 font-black italic">{totalReturn}건</p></div>
                   <div><p className="text-[10px] text-slate-400 uppercase mb-2">전체 반품율</p><p className="text-lg md:text-3xl text-rose-600 font-black italic">{getRate(totalReturn, totalDB)}%</p></div>
                </div>

                <div className="space-y-4">
                  <p className="text-xs text-slate-400 italic uppercase ml-2">Team Total Activity</p>
                ...
                <div className="space-y-4">
                  <p className="text-xs text-slate-400 italic uppercase ml-2">Team Conversion Funnel (%)</p>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-6">
              ...
              <div className="space-y-6 md:space-y-10 animate-in slide-in-from-right-4 duration-300">
                <div className="flex flex-col md:flex-row justify-between items-center bg-black p-6 md:p-8 rounded-2xl text-white gap-4">
                   ...
                   <button onClick={() => handleApprove(selectedAgent.id, selectedAgent.performance.is_approved)} className={`w-full md:w-auto px-6 py-3 rounded-full text-sm font-black italic shadow-xl transition-transform active:scale-95 ${selectedAgent.performance.is_approved ? 'bg-rose-600 text-white' : 'bg-[#d4af37] text-black'}`}>
                    {selectedAgent.performance.is_approved ? 'REVOKE LOCK' : 'CONFIRM & LOCK'}
                   </button>
                </div>

                <div className="grid grid-cols-3 gap-2 md:gap-6">
                   <div className="bg-blue-50 p-4 md:p-8 rounded-2xl border-2 md:border-4 border-black text-center"><p className="text-[10px] text-blue-400 uppercase mb-2">배정 DB</p><p className="text-lg md:text-3xl text-blue-900 italic font-black">{selectedAgent.performance.db_assigned || 0}건</p></div>
                   <div className="bg-rose-50 p-4 md:p-8 rounded-2xl border-2 md:border-4 border-black text-center"><p className="text-[10px] text-rose-400 uppercase mb-2">반품 DB</p><p className="text-lg md:text-3xl text-rose-900 italic font-black">{selectedAgent.performance.db_returned || 0}건</p></div>
                   <div className="bg-slate-900 p-4 md:p-8 rounded-2xl border-2 md:border-4 border-black text-center"><p className="text-[10px] text-slate-500 uppercase mb-2">반품율</p><p className="text-lg md:text-3xl text-[#d4af37] italic font-black">{getRate(selectedAgent.performance.db_returned, selectedAgent.performance.db_assigned)}%</p></div>
                </div>

                <div className="space-y-4">
                  <p className="text-xs text-slate-400 italic uppercase ml-2">Detailed Activity Metrics</p>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-6">
            ...
            <div className="border-2 md:border-4 border-black rounded-2xl md:rounded-[2.5rem] overflow-hidden font-black">
              <table className="w-full text-left font-black">
                <thead className="bg-black text-[#d4af37] text-[9px] md:text-[10px] uppercase font-black">
                  <tr>
                    <th className="p-3 md:p-6">Name</th>
                    <th className="p-3 md:p-6 text-center">Status</th>
                    <th className="p-3 md:p-6 text-center">1W</th>
                    <th className="p-3 md:p-6 text-center">2W</th>
                    <th className="p-3 md:p-6 text-center">3W</th>
                    <th className="p-3 md:p-6 text-center">4W</th>
                    <th className="p-3 md:p-6 text-center">Plus</th>
                  </tr>
                </thead>
                <tbody className="divide-y-2 divide-black">
                  {agents.map((a:any) => (
                    <tr key={a.id} className="hover:bg-slate-50">
                      <td className="p-3 md:p-6 font-black text-xs md:text-lg italic">{a.name} CA</td>
                      <td className="p-3 md:p-6 text-center">
                      ...
                      {[1, 2, 3, 4, 5].map((w) => (
                        <td key={w} className="p-1 md:p-6 text-center">
                          <div className={`w-6 h-6 md:w-8 md:h-8 mx-auto rounded-lg border-2 flex items-center justify-center font-black ${a.performance[`edu_${w}`] ? 'bg-black text-[#d4af37] border-black' : 'bg-white border-slate-200'}`}>✓</div>
                        </td>
                      ))}
                    </tr>
                  ))}
...
function ActivityCountBox({ label, val }: any) { 
  return (
    <div className="bg-white p-3 md:p-6 rounded-2xl border-2 md:border-4 border-black text-center font-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      <p className="text-[9px] text-slate-400 uppercase mb-1 font-black">{label}</p>
...
function StatBox({ label, cur, tar, unit, color }: any) { 
  ...
  return (
    <div className="text-center space-y-2 md:space-y-6 font-black p-3 md:p-4">
      ...
      <div className="w-full h-6 md:h-10 bg-slate-100 rounded-full border-2 md:border-4 border-black overflow-hidden relative font-black shadow-lg">
...
function FunnelBox({ label, val }: any) { 
  return (
    <div className="p-3 md:p-6 bg-slate-50 rounded-2xl border-2 md:border-4 border-black border-dashed text-center font-black">
...
function InputRow({ label, val, onChange }: any) { 
  return (
    <div className="flex justify-between items-center bg-slate-50 p-3 md:p-6 rounded-2xl border-2 md:border-4 border-black font-black shadow-md">
