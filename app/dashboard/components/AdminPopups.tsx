...
export default function AdminPopups({ type, agents, selectedAgent, teamMeta, onClose }: any) {
  ...
  const [allUsers, setAllUsers] = useState<any[]>([]); // 전체 유저 관리를 위해 변경
  ...
  useEffect(() => {
    async function load() {
      ...
      // 2. 조직 정보 및 전체 유저 로드 (승인 여부 상관없이)
      if (type === 'users') {
        const { data: uData } = await supabase
          .from("users")
          .select("*")
          .order("is_approved", { ascending: true }) // 미승인자가 위로 오게
  ...
  return (
    <div className="fixed inset-0 z-[500] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 font-black text-black">
      <div className="bg-white w-full max-w-6xl rounded-[2.5rem] md:rounded-[4rem] p-6 md:p-12 relative overflow-y-auto max-h-[90vh] border-4 border-black shadow-2xl">
        ...
        {/* 1. User Management (전체 직원 관리 및 소속 수정) */}
        {type === 'users' && (
                    ...
                    <tr key={u.id} className={`${u.is_approved ? 'bg-white' : 'bg-amber-50'} hover:bg-slate-50 transition-colors`}>
                      ...
                      <td className="p-4 md:p-6 text-center">
                        <div className="flex flex-col gap-2 max-w-[250px] mx-auto">
                          {u.isCustomDept ? (
                            <input type="text" placeholder="사업부 입력" className="p-2 border-2 border-indigo-500 rounded-xl text-xs font-black" onChange={(e) => updateUserInfo(u.id, 'department', e.target.value)} />
                          ) : (
                          ...
                          {u.isCustomTeam ? (
                            <input type="text" placeholder="지점 입력" className="p-2 border-2 border-indigo-500 rounded-xl text-xs font-black" onChange={(e) => updateUserInfo(u.id, 'team', e.target.value)} />
                          ) : (
        ...
        {/* 2. Team Performance (실적 리스트 및 지표) */}
        {type === 'perf' && (
          <div className="space-y-6 md:space-y-10 animate-in fade-in">
        ...
        {/* 5. System Settings (교육 입력란 추가 완료) */}
        {type === 'sys' && (
          <div className="space-y-6 md:space-y-10 font-black animate-in fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              ...
              <div className="space-y-4">
                ...
                <p className="text-[9px] text-slate-400 italic mt-2">* 저장된 내용은 직원의 메인 화면 교육 세션에 즉시 노출됩니다.</p>
              </div>
            </div>
          </div>
        )}
