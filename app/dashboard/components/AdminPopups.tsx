...
export default function AdminPopups({ type, agents, selectedAgent, teamMeta, onClose }: any) {
  ...
  // 유저 정보 실시간 업데이트 (직접 입력 오류 해결의 핵심)
  const updateUserInfo = (userId: string, field: string, value: string) => {
    setAllUsers(prev => prev.map(u => {
      ...
      if (field === 'team') {
        if (value === 'CUSTOM_INPUT') return { ...u, isCustomTeam: true, team: '' };
        return { ...u, team: value, isCustomTeam: false };
      }
  ...
  const handleApprovePerf = async (agentId: string, currentStatus: boolean) => {
    ...
    else { alert(!currentStatus ? "승인되었습니다." : "승인 해제되었습니다."); onClose(); }
  };

  // 직원 정보 저장
  const handleUserSave = async (user: any) => {
    if (!user.department?.trim() || !user.team?.trim()) {
      alert("사업부와 지점을 정확히 입력해주세요.");
      return;
    }
              ...
              <table className="w-full text-left font-black">
                <thead className="bg-black text-[#d4af37] text-[10px] uppercase">
                  <tr>
                    ...
                    <th className="p-4 md:p-6 text-center">Department / Team</th>
                    ...
                    <tr key={u.id} className={`${u.is_approved ? 'bg-white' : 'bg-amber-50'} hover:bg-slate-50 transition-colors`}>
                      ...
                      <td className="p-4 md:p-6 text-center">
                        <div className="flex flex-col gap-2 max-w-[250px] mx-auto">
                          ...
                          {u.isCustomTeam ? (
                            <input type="text" placeholder="지점 입력" className="p-2 border-2 border-indigo-500 rounded-xl text-xs font-black bg-indigo-50" value={u.team || ""} onChange={(e) => updateUserInfo(u.id, 'team', e.target.value)} />
                          ) : (
                            <select value={u.team || ""} onChange={(e) => updateUserInfo(u.id, 'team', e.target.value)} disabled={!u.department && !u.isCustomDept} className="p-2 bg-slate-100 border-2 border-black rounded-xl text-xs font-black disabled:opacity-30">
            ...
            {selectedAgent ? (
              <div className="space-y-6">
                <div className="bg-black p-6 rounded-2xl text-white flex justify-between items-center shadow-2xl">
                  <p className="text-xl md:text-3xl italic underline decoration-[#d4af37] underline-offset-8">{selectedAgent.name} CA Analysis</p>
                ...
                <div className="grid grid-cols-3 gap-4 font-black">
                  <div className="bg-blue-50 p-6 rounded-2xl border-4 border-black text-center">
                    ...
                    <p className="text-2xl italic">{selectedAgent.performance.db_assigned || 0}건</p>
                  </div>
                  <div className="bg-rose-50 p-6 rounded-2xl border-4 border-black text-center font-black">
                    ...
                    <p className="text-2xl italic text-rose-600">{selectedAgent.performance.db_returned || 0}건</p>
                  </div>
                  <div className="bg-slate-900 p-6 rounded-2xl border-4 border-black text-center text-[#d4af37] font-black">
                    ...
                    <p className="text-2xl italic">{getRate(selectedAgent.performance.db_returned, selectedAgent.performance.db_assigned)}%</p>
                  </div>
                </div>
          ...
          <div className="space-y-6 md:space-y-10 animate-in fade-in">
            ...
            <div className="border-4 border-black rounded-[2.5rem] overflow-x-auto shadow-xl">
              <table className="w-full text-left min-w-[600px] font-black">
                ...
                <tbody className="divide-y-2 divide-black">
                  {agents.map((a:any) => (
                    <tr key={a.id} className="hover:bg-slate-50 transition-colors font-black">
                      <td className="p-6 font-black text-lg italic sticky left-0 bg-white border-r-2 border-black/5">{a.name} CA</td>
                      {[1, 2, 3, 4, 5].map((w) => (
                        <td key={w} className="p-6 text-center font-black">
                          <div className={`w-8 h-8 mx-auto rounded-lg border-2 flex items-center justify-center font-black ${a.performance[`edu_${w}`] ? 'bg-black text-[#d4af37] border-black' : 'border-slate-200 text-transparent'}`}>✓</div>
                        </td>
                      ))}
                    </tr>
                  ))}
