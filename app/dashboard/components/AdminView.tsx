...
export default function AdminView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  ...
  async function fetchTeamData() {
    ...
    const gNotice = settings?.find(s => s.key === 'global_notice')?.value || "등록된 공지사항이 없습니다.";
    const gAmt = Number(settings?.find(s => s.key === 'team_target_amt')?.value || 0);
    const gCnt = Number(settings?.find(s => s.key === 'team_target_cnt')?.value || 0);
    const gIntro = Number(settings?.find(s => s.key === 'team_target_intro')?.value || 0);

    setGlobalNotice(gNotice);
    setTeamMeta({ targetAmt: gAmt, targetCnt: gCnt, targetIntro: gIntro });
    ...
    <div className="space-y-6 animate-in fade-in pb-20">
      ...
      <section className="bg-white p-8 rounded-[3.5rem] border shadow-sm">
        ...
        <div className="space-y-3">
          {agents.map(a => (
            <div key={a.id} onClick={() => setSelectedAgent(a)} className="bg-slate-50 p-6 rounded-[2rem] hover:border-black border-2 border-transparent transition-all cursor-pointer flex flex-col lg:flex-row lg:items-center gap-6 font-black">
                <div className="w-32 shrink-0">
                  <p className="text-lg">{a.name} CA</p>
                  <span className={`text-[9px] px-2 py-1 rounded-full ${a.performance?.edu_status === '참여' ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-200 text-slate-400'}`}>교육 {a.performance?.edu_status || '미참여'}</span>
                </div>
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-white p-3 rounded-2xl border text-center font-black">
                    ...
                    <p className="text-sm text-indigo-600">{a.performance?.contract_amt || 0}만 / <span className="text-slate-400">{a.performance?.target_amt || 0}만</span></p>
                  </div>
                  <div className="bg-white p-3 rounded-2xl border text-center font-black">
                    <p className="text-[8px] text-slate-400">건수 (실적 / 목표)</p>
                    <p className="text-sm text-emerald-600">{a.performance?.contract_cnt || 0}건 / <span className="text-slate-400">{a.performance?.target_cnt || 0}건</span></p>
                  </div>
                </div>
            </div>
          ))}
      ...
      {/* 팝업이 닫힐 때 fetchTeamData를 실행하도록 설정 */}
        ...
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 font-black" onClick={()=>setSelectedAgent(null)}>
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-10 relative shadow-2xl" onClick={e=>e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8 text-center">
                <div className="bg-slate-50 p-4 rounded-2xl border"><p className="text-[10px] text-slate-400">전화</p><p className="text-xl">{selectedAgent.performance?.call || 0}</p></div>
                <div className="bg-slate-50 p-4 rounded-2xl border"><p className="text-[10px] text-slate-400">만남</p><p className="text-xl">{selectedAgent.performance?.meet || 0}</p></div>
                <div className="bg-slate-50 p-4 rounded-2xl border"><p className="text-[10px] text-slate-400">제안</p><p className="text-xl">{selectedAgent.performance?.pt || 0}</p></div>
                <div className="bg-slate-50 p-4 rounded-2xl border"><p className="text-[10px] text-slate-400">소개</p><p className="text-xl">{selectedAgent.performance?.intro || 0}</p></div>
            </div>
