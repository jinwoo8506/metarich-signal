...
export default function AdminView({ user, selectedDate }: { user: any, selectedDate: Date }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in pb-20 font-black">
      ...
      {/* 관리 탭 메뉴 */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 font-black font-black">
      ...
      <section className="bg-white p-8 rounded-[3.5rem] border shadow-sm font-black">
        <h2 className="text-xl font-black mb-6 border-l-8 border-black pl-4 uppercase">{selectedDate.getMonth()+1}월 팀 모니터링</h2>
          ...
          {agents.map(a => (
            <div key={a.id} 
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3 font-black">
                    <DataBox label="실적액" val={`${a.performance.contract_amt}만`} />
                    <DataBox label="실적건" val={`${a.performance.contract_cnt}건`} />
      ...
      {selectedAgent && (
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 font-black" onClick={()=>setSelectedAgent(null)}>
          <div className="bg-white w-full max-w-xl rounded-[3.5rem] p-10 relative shadow-2xl font-black" onClick={e=>e.stopPropagation()}>
            ...
            <h3 className="text-2xl italic mb-8 border-b-4 border-black pb-2 inline-block uppercase font-black font-black">{selectedAgent.name} CA 세부 현황</h3>
            <div className="grid grid-cols-2 gap-3 mb-8 text-center font-black">
                <div className="bg-slate-50 p-6 rounded-3xl border font-black font-black"><p className="text-[10px] text-slate-400 mb-1 font-black">CALL</p><p className="text-3xl font-black italic">{selectedAgent.performance?.call || 0}</p></div>
                <div className="bg-slate-50 p-6 rounded-3xl border font-black font-black"><p className="text-[10px] text-slate-400 mb-1 font-black">MEET</p><p className="text-3xl font-black italic">{selectedAgent.performance?.meet || 0}</p></div>
                <div className="bg-slate-50 p-6 rounded-3xl border font-black font-black"><p className="text-[10px] text-slate-400 mb-1 font-black">PT</p><p className="text-3xl font-black italic">{selectedAgent.performance?.pt || 0}</p></div>
                <div className="bg-slate-50 p-6 rounded-3xl border font-black font-black"><p className="text-[10px] text-slate-400 mb-1 font-black">INTRO</p><p className="text-3xl font-black italic">{selectedAgent.performance?.intro || 0}</p></div>
            </div>
            <div className="bg-slate-900 text-[#d4af37] p-8 rounded-[2.5rem] flex justify-between items-center font-black">
              ...
              <span className="text-2xl font-black italic font-black font-black">반품 {selectedAgent.performance?.db_returned || 0} / 배정 {selectedAgent.performance?.db_assigned || 0}</span>
            </div>
          </div>
        </div>
      )}
...
function DataBox({ label, val }: any) { return <div className="bg-white p-3 rounded-2xl border text-center font-black font-black font-black"><p className="text-[9px] text-slate-400 font-black">{label}: {val}</p></div> }