...
export default function DashboardPage() {
  ...
  // 📅 [중요] TypeScript 비교 에러 방지를 위한 타입 명시
  ...
  // 🛡️ [수정] getAlertStyle: 데이터 부재 시 기본값 부여로 타입 에러 방지
          ...
          <div className="bg-white w-full max-w-5xl rounded-[4rem] p-14 relative overflow-y-auto max-h-[90vh] border-[10px] border-black">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-8">
                  <InBox label="목표 건수" value={(selectedAgent.monthly_targets || []).find(mt=>mt.year===year && mt.month===month)?.target_count || 0} onChange={()=>{}} unit="건" />
                  <InBox label="목표 금액" value={(selectedAgent.monthly_targets || []).find(mt=>mt.year===year && mt.month===month)?.target_amount || 0} onChange={()=>{}} unit="만원" />
                </div>
              </div>
              <button onClick={() => setSelectedAgent(null)} className="w-full bg-black text-[#d4af37] py-8 rounded-[3rem] font-black text-2xl mt-12 uppercase">Save & Close</button>
          </div>
        </div>
      )}
