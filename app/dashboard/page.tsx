...
export default function IntegratedBusinessDashboard() {
  ...
  async function fetchDailyData() {
    ...
    else { setPerfInput({ ...perfInput, call: 0, meet: 0, pt: 0, intro: 0, db_assigned: 0, db_returned: 0, contract_cnt: 0, contract_amt: 0, edu_done: false }); setIsTargetLocked(false); }
  }
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col lg:flex-row font-sans text-slate-900 overflow-x-hidden">
      ...
      <aside className="w-full lg:w-80 bg-white border-b lg:border-r p-6 flex flex-col gap-6 shadow-sm z-40">
        ...
        <div className="bg-slate-50 p-5 rounded-[2rem] border border-dashed border-slate-300">
            <p className="text-[10px] font-black text-slate-400 uppercase text-center mb-3">3개월 월평균 데이터</p>
            {sideTab === 'perf' ? (
                <div className="space-y-4">
                    <div className="flex justify-between items-end"><span className="text-[10px] font-bold text-slate-400">평균 실적</span><span className="text-lg font-black italic">{(perfInput.contract_amt || 0).toLocaleString()}만</span></div>
                    <div className="flex justify-between items-end"><span className="text-[10px] font-bold text-slate-400">평균 건수</span><span className="text-lg font-black italic">{(perfInput.contract_cnt || 0)}건</span></div>
                </div>
            ) : (
        ...
        {(role === 'admin' || role === 'master') ? (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <TabBtn label="실적 관리" sub="팀 목표 및 달성률" onClick={()=>setActiveAdminPopup('perf')} />
              <TabBtn label="활동 관리" sub="DB 및 활동 효율" onClick={()=>setActiveAdminPopup('act')} />
              <TabBtn label="교육 관리" sub="이수 명단 확인" onClick={()=>setActiveAdminPopup('edu')} />
              <TabBtn label="시스템 설정" sub="Configuration" onClick={()=>setActiveAdminPopup('setting')} />
            </div>

            <section className="bg-white p-8 rounded-[3.5rem] shadow-sm border">
                <h2 className="text-xl font-black uppercase italic mb-6 border-l-8 border-black pl-4">Agent Live Board</h2>
                        ...
                        <div key={a.id} onClick={()=>setSelectedAgent(a)} className="bg-slate-50 p-6 rounded-[2.5rem] border-2 border-transparent hover:border-black cursor-pointer transition-all">
                            ...
                            {a.performance ? (
                                <div className="space-y-3">
                                    ...
                                    <MiniBar label="목표 달성" val={Math.min((a.performance.contract_amt/a.performance.target_amt)*100, 100)} color="bg-black" />
                                </div>
                            ) : <p className="text-[10px] font-bold text-slate-400 italic uppercase">No data reported</p>}
                        </div>
                    ))}
                </div>
            </section>
          ...
          /* 👤 [AGENT VIEW: 가로사이즈 최적화 및 저장 버튼 분리] */
          <div className="max-w-4xl mx-auto space-y-8">
            <header className="px-4">
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest">{userName} CA</p>
              <h1 className="text-4xl font-black italic uppercase">Performance Input</h1>
            </header>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-sm border">
                <h3 className="text-xs font-black text-slate-400 uppercase mb-8 text-center tracking-widest">Daily Activity</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                    <MetricInput label="전화" val={perfInput.call} onChange={(v)=>setPerfInput({...perfInput, call:v})} />
                    <MetricInput label="만남" val={perfInput.meet} onChange={(v)=>setPerfInput({...perfInput, meet:v})} />
                ...
                <button onClick={savePerformance} className="w-full mt-10 bg-black text-white py-6 rounded-3xl font-black uppercase text-lg hover:invert transition-all">실적 데이터 저장 (관리자 연동)</button>
            </div>

            <div className="bg-indigo-50 p-10 rounded-[3rem] border-2 border-indigo-100 space-y-6">
                <h3 className="text-xl font-black italic uppercase text-indigo-900 flex items-center gap-2"><span>📚</span> Weekly Education</h3>
                <div className="bg-white p-6 rounded-2xl border border-indigo-200">
                  <p className="text-sm font-bold text-slate-800">"고객 거절 화법 극복 및 암보험 신상품 비교 교육"</p>
                </div>
                <label className="flex items-center gap-3 bg-white p-5 rounded-2xl cursor-pointer">
                    <input type="checkbox" checked={perfInput.edu_done} onChange={(e)=>setPerfInput({...perfInput, edu_done: e.target.checked})} className="w-6 h-6 border-indigo-600 text-indigo-600" />
                    <span className="font-black text-indigo-900 italic uppercase">본인은 위 교육을 이수하였음을 확인합니다.</span>
                </label>
                <button onClick={saveEducation} className="w-full bg-indigo-600 text-white py-5 rounded-3xl font-black uppercase text-lg hover:bg-indigo-700 transition-all">교육 이수 여부 저장</button>
            </div>
          </div>
        )}
      </main>

      {/* ─── 🧱 [MODALS: 관리자 탭 상세 기능 복구] ────────────────────────── */}
      {activeAdminPopup && (
          ...
          <div className="bg-white w-full max-w-5xl rounded-[3rem] p-10 relative overflow-y-auto max-h-[90vh]">
            ...
            {activeAdminPopup === 'perf' && (
                <div className="space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="h-[400px] bg-slate-50 p-6 rounded-3xl">
                        ...
                        <div className="space-y-6">
                            <StatCard label="팀 전체 실적" val={`${agents.reduce((acc, a) => acc + (a.performance?.contract_amt || 0), 0).toLocaleString()}만`} />
            ...
            {activeAdminPopup === 'act' && (
                <div className="space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                        <div className="h-[400px] flex items-center justify-center bg-slate-50 rounded-3xl">
                            <div className="w-[300px]"><Pie data={{ labels: ['정상DB', '반품'], datasets: [{ data: [agents.reduce((acc, a)=>acc+(a.performance?.db_assigned||0), 0), agents.reduce((acc, a)=>acc+(a.performance?.db_returned||0), 0)], backgroundColor: ['#000', '#ff4d4d'] }] }} options={{ maintainAspectRatio: false }} /></div>
                        </div>
                        <div className="h-[400px] bg-slate-50 p-6 rounded-3xl">
                            <Line data={{ labels: ['전화', '만남', '제안'], datasets: [{ label: '활동량', data: [agents.reduce((acc, a)=>acc+(a.performance?.call||0), 0), agents.reduce((acc, a)=>acc+(a.performance?.meet||0), 0), agents.reduce((acc, a)=>acc+(a.performance?.pt||0), 0)], borderColor: '#000' }] }} options={{ maintainAspectRatio: false }} />
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeAdminPopup === 'edu' && (
                <div className="space-y-10">
                    ...
                    <div className="bg-slate-50 p-8 rounded-3xl space-y-4">
                        {agents.map(a => (
                            <div key={a.id} className="flex justify-between items-center p-4 bg-white rounded-2xl border">
                                <span className="font-black text-lg">{a.name} CA</span>
                                <span className={`font-black px-4 py-1 rounded-full text-xs ${a.performance?.edu_done ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}`}>
                                    {a.performance?.edu_done ? '이수 완료' : '미이수'}
                                </span>
                            </div>
                        ))}
                    </div>
      ...
      {/* 👤 [SELECTED AGENT DETAIL: 클릭 시 즉시 활동/실적 노출] */}
      {selectedAgent && (
        <div className="fixed inset-0 z-[210] bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-2xl rounded-[3.5rem] p-10 relative">
                <button onClick={()=>setSelectedAgent(null)} className="absolute top-10 right-10 text-2xl font-black">✕</button>
                <div className="space-y-10">
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 bg-black rounded-full flex items-center justify-center text-[#d4af37] font-black text-3xl">{selectedAgent.name.charAt(0)}</div>
                        <div>
                            <h2 className="text-3xl font-black uppercase italic">{selectedAgent.name} CA</h2>
                            <p className="text-xs font-bold text-slate-400">Current Performance Detail</p>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <StatItem label="CALL" val={selectedAgent.performance?.call} />
                        <StatItem label="MEET" val={selectedAgent.performance?.meet} />
                        <StatItem label="DB ASSIGNED" val={selectedAgent.performance?.db_assigned} />
                        <StatItem label="DB RETURNED" val={selectedAgent.performance?.db_returned} />
                    </div>
                    <div className="bg-slate-50 p-8 rounded-[2.5rem] space-y-4">
                        <div className="flex justify-between font-black uppercase italic"><span>Monthly Amt</span><span className="text-2xl">{selectedAgent.performance?.contract_amt}만</span></div>
                        <div className="flex justify-between font-black uppercase italic"><span>Monthly Cnt</span><span className="text-2xl">{selectedAgent.performance?.contract_cnt}건</span></div>
                        <MiniBar label="Goal Achievement" val={selectedAgent.performance ? (selectedAgent.performance.contract_amt/selectedAgent.performance.target_amt)*100 : 0} color="bg-black" />
                    </div>
                </div>
            </div>
        </div>
...
// ─── 📦 [SUB-COMPONENTS] ──────────────────────────
function QuickBtn({ label, color, onClick, url }: any) {
...
function TabBtn({ label, sub, onClick }: any) {
    return (
        <button onClick={onClick} className="bg-white border-2 border-black p-6 rounded-[2.5rem] text-center hover:bg-black group transition-all active:scale-95">
...
function MiniBar({ label, val, color }: any) {
  return (
    <div className="w-full">
      ...
      <div className="w-full h-2.5 bg-slate-200 rounded-full overflow-hidden"><div className={`h-full ${color}`} style={{ width: `${val}%` }} /></div>
    </div>
  )
}
...
function ProgCard({ label, cur, tar, unit, color, isLocked, onChangeCur, onChangeTar }: any) {
        ...
        <div className="bg-white p-10 rounded-[3rem] border shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                    ...
                    <input type="number" value={tar} disabled={isLocked} onChange={e=>onChangeTar(Number(e.target.value))} className={`w-full p-4 bg-slate-50 rounded-2xl font-black text-center border-2 outline-none text-xl ${isLocked ? 'opacity-50 cursor-not-allowed' : 'border-dashed'}`} />
                </div>
                <div className="space-y-1">
            ...
            <div className="space-y-2">
                ...
                <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full ${color.replace('text-', 'bg-')} transition-all duration-1000`} style={{ width: `${pct}%` }} /></div>
            </div>
        </div>
    )
}
...
function StatCard({ label, val }: any) {
    return <div className="bg-slate-50 p-6 rounded-3xl border"><p className="text-xs font-black text-slate-400 uppercase mb-1">{label}</p><p className="text-3xl font-black italic uppercase">{val}</p></div>
}

function StatItem({ label, val }: any) {
    return <div className="p-5 bg-slate-50 rounded-2xl border text-center"><p className="text-[10px] font-black text-slate-400 uppercase mb-1">{label}</p><p className="text-xl font-black">{val || 0}</p></div>
}
...
function FinanceCalculator() {
    ...
    return (
        <div className="p-8 bg-slate-100 rounded-[3rem] space-y-6">
            <h4 className="font-black italic uppercase">Compound Calculator</h4>
            <div className="space-y-4">
                <div className="grid grid-cols-1 gap-4">
                    <input type="number" value={p} onChange={e=>setP(Number(e.target.value))} className="p-4 rounded-xl border-none font-black text-center" placeholder="원금(만)" />
                    <div className="grid grid-cols-2 gap-4">
                        <input type="number" value={r} onChange={e=>setR(Number(e.target.value))} className="p-4 rounded-xl border-none font-black text-center" placeholder="수익률(%)" />
                        <input type="number" value={t} onChange={e=>setT(Number(e.target.value))} className="p-4 rounded-xl border-none font-black text-center" placeholder="기간(년)" />
                    </div>
                </div>
                <div className="bg-black p-6 rounded-2xl text-center"><p className="text-xs font-black text-[#d4af37] uppercase mb-1">Estimated Value</p><p className="text-3xl font-black text-white italic">{res.toLocaleString()} 만원</p></div>
            </div>
        </div>
    )
}