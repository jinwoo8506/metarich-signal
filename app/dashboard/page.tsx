      ...
      <main className="flex-1 p-6 lg:p-10 space-y-8 max-w-[1600px] mx-auto w-full">
        ...
        {role === "agent" && (
          <div className="space-y-8 animate-in fade-in duration-700">
            {/* 1. 목표 및 현재 실적 (그래프형 시각화) */}
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
               <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
                  <h3 className="text-sm font-black text-slate-400 mb-6 uppercase tracking-widest">Target Achievement (Amount)</h3>
                  <div className="space-y-6">
                     <div className="grid grid-cols-2 gap-4">
                        <InputUnit label="목표 금액" val={myPerf.target_amount} onChange={(v)=>setMyPerf({...myPerf, target_amount: v})} color="text-slate-400" />
                        <InputUnit label="현재 금액" val={myPerf.contract_amount} onChange={(v)=>setMyPerf({...myPerf, contract_amount: v})} color="text-indigo-600" />
                     </div>
                     <BigProgressBar label="금액 달성률" current={myPerf.contract_amount} target={myPerf.target_amount} color="bg-indigo-500" />
                  </div>
               </div>
               <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
                  <h3 className="text-sm font-black text-slate-400 mb-6 uppercase tracking-widest">Target Achievement (Count)</h3>
                  <div className="space-y-6">
                     <div className="grid grid-cols-2 gap-4">
                        <InputUnit label="목표 건수" val={myPerf.target_count} onChange={(v)=>setMyPerf({...myPerf, target_count: v})} color="text-slate-400" />
                        <InputUnit label="현재 건수" val={myPerf.contract_count} onChange={(v)=>setMyPerf({...myPerf, contract_count: v})} color="text-emerald-500" />
                     </div>
            ...
            {/* 2. 활동 지표 입력 (가로 1줄 배열) */}
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100">
               <h3 className="text-sm font-black text-slate-400 mb-6 uppercase tracking-widest">Activity Metrics (Daily)</h3>
               <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
                  <InputUnit label="전화" val={myPerf.call_count} onChange={(v)=>setMyPerf({...myPerf, call_count: v})} />
                  <InputUnit label="미팅" val={myPerf.meet_count} onChange={(v)=>setMyPerf({...myPerf, meet_count: v})} />
                  <InputUnit label="제안" val={myPerf.pt} onChange={(v)=>setMyPerf({...myPerf, pt: v})} />
                  <InputUnit label="도입" val={myPerf.intro_count} onChange={(v)=>setMyPerf({...myPerf, intro_count: v})} />
                  <InputUnit label="DB배정" val={myPerf.db_assigned} onChange={(v)=>setMyPerf({...myPerf, db_assigned: v})} color="text-blue-600" />
                  <InputUnit label="반품" val={myPerf.db_returned} onChange={(v)=>setMyPerf({...myPerf, db_returned: v})} color="text-rose-500" />
               </div>
               <button className="w-full mt-8 bg-black text-white py-5 rounded-2xl font-black uppercase hover:opacity-80 transition-all">Save All Records</button>
            </div>
          </div>
        )}
        ...
        {(role === "admin" || role === "master") && (
          <section className="space-y-8">
            <h2 className="text-xl font-black italic border-l-8 border-black pl-4 uppercase">Team Performance Analysis</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {agents.filter(a => a.role === 'agent').map(a => {
                const p = a.performances?.[0] || myPerf; // 예시 데이터 매핑
                return (
                  <div key={a.id} className="bg-white p-8 rounded-[3rem] shadow-md border-2 border-transparent hover:border-black transition-all">
                    <div className="flex justify-between items-center mb-8">
                      <p className="text-2xl font-black uppercase tracking-tighter">{a.name} <span className="text-slate-300 text-sm italic">Status</span></p>
                      <div className="flex gap-2">
                        <span className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-full uppercase">Active</span>
                      </div>
                    </div>
                    {/* 관리자용 막대 그래프 비교 */}
                    <div className="space-y-6">
                      <AdminBarChart label="매출 달성" current={p.contract_amount} target={p.target_amount} unit="원" color="bg-indigo-500" />
                      <AdminBarChart label="건수 달성" current={p.contract_count} target={p.target_count} unit="건" color="bg-emerald-500" />
                    </div>
                    <div className="grid grid-cols-6 gap-2 mt-8 pt-6 border-t border-slate-100">
                      ...
                      <MiniStat label="제안" val={p.pt} />
                      <MiniStat label="도입" val={p.intro_count} />
                      <MiniStat label="DB" val={p.db_assigned} color="text-blue-600" />
                      <MiniStat label="반품" val={p.db_returned} color="text-rose-500" />
                    </div>
                  </div>
                )
              })}
...
function BigProgressBar({ label, current, target, color }: any) {
...
function AdminBarChart({ label, current, target, unit, color }: any) {
  const percent = target > 0 ? Math.min((current / target) * 100, 100) : 0;
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-[10px] font-black uppercase">
        <span>{label}</span>
        <span>{current.toLocaleString()}{unit} / {target.toLocaleString()}{unit}</span>
      </div>
      <div className="relative w-full h-8 bg-slate-50 rounded-lg border border-slate-100 overflow-hidden">
        <div className={`absolute h-full ${color} opacity-20 transition-all duration-1000`} style={{ width: `${percent}%` }} />
        <div className={`absolute h-full ${color} w-1 top-0 transition-all duration-1000`} style={{ left: `${percent}%` }} />
        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-black italic">{percent.toFixed(1)}% Completed</span>
      </div>
    </div>
  )
}
...
function MemoBox({ label, value, onChange, readOnly, color }: any) {
  return (
    <div className={`${color} p-6 rounded-[2rem] border border-slate-100 shadow-sm`}>
        ...
        <textarea readOnly={readOnly} value={value} onChange={onChange} className="w-full bg-transparent text-xs font-bold outline-none resize-none h-28 text-slate-700" />
    </div>
  )
}