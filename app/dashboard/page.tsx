    ...
    <div className="min-h-screen bg-[#f1f5f9] flex text-slate-900 font-sans text-base md:text-lg">
      ...
      <main className="flex-1 p-5 md:p-14 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-8 md:space-y-12">
          ...
          <header className="bg-black text-white p-6 md:p-10 rounded-[2rem] md:rounded-[4rem] flex flex-col md:flex-row justify-between items-center gap-6 shadow-2xl border-b-4 md:border-b-8 border-[#d4af37]">
            <div className="text-center md:text-left"><h1 className="text-2xl md:text-4xl font-black italic text-[#d4af37] tracking-tighter">METARICH SIGNAL</h1><p className="text-[10px] text-white/40 font-bold uppercase mt-1">v4.0 Admin Fixed</p></div>
                    ...
                    {agents.map(a => {
                        ...
                        return (
                            <div key={a.id} onClick={() => { setSelectedAgent(a); setEditingComment(t.admin_comment || ""); }}
                                 className="bg-white p-6 md:p-10 rounded-[2rem] md:rounded-[4rem] border-4 border-white hover:border-black transition-all cursor-pointer shadow-lg group">
          ...
          {(role === "agent" || role === "master") && (
             <section className="space-y-8 md:space-y-12">
               <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                 ...
                 <button onClick={handleAgentSave} className="w-full md:w-auto bg-black text-[#d4af37] px-8 py-4 md:px-10 md:py-5 rounded-2xl font-black text-xs md:text-sm">UPDATE PERFORMANCE</button>
               </div>
               ...
               <div className="bg-white p-6 md:p-12 rounded-[2.5rem] md:rounded-[5rem] shadow-xl border-4 border-slate-50 grid grid-cols-1 md:grid-cols-2 gap-10">
                 <div className="space-y-6 md:space-y-10">
                   <h3 className="text-xl md:text-2xl font-black text-slate-800 uppercase border-l-8 border-slate-300 pl-4 italic">Goal & Recruit</h3>
                 ...
                 <div className="space-y-6 md:space-y-10">
                   <h3 className="text-xl md:text-2xl font-black text-[#d4af37] uppercase border-l-8 border-[#d4af37] pl-4 italic">Current Result</h3>
      ...
      {isNoticeOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 relative border-4 border-[#d4af37]">
            <button onClick={()=>setIsNoticeOpen(false)} className="absolute top-6 right-6 font-black text-2xl">✕</button>
            <h2 className="text-xl md:text-2xl font-black mb-6 italic border-b-4 border-black pb-2 uppercase text-[#d4af37]">Global Notice</h2>
            <div className="text-base font-bold leading-relaxed whitespace-pre-wrap py-4">{globalNotice}</div>
      ...
      {selectedAgent && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-[150] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl rounded-[2rem] md:rounded-[4rem] p-6 md:p-14 relative overflow-y-auto max-h-[90vh] border-4 md:border-[10px] border-black">
              ...
              <h2 className="text-2xl md:text-5xl font-black mb-8 italic uppercase border-b-8 border-black pb-4">{selectedAgent.name} Coaching</h2>
              
              {/* 📞 상세 활동 데이터 복구 섹션 */}
              <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-10 bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100">
              ...
              }} className="w-full bg-black text-[#d4af37] py-6 rounded-[2rem] font-black text-xl md:text-3xl mt-8">SAVE COACHING</button>
          </div>
        </div>
      )}
      ...
      {isTeamGoalModalOpen && (
        <div className="fixed inset-0 bg-black/90 z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-xl rounded-[2rem] p-8 md:p-16 relative border-4 border-[#d4af37]">
            <button onClick={() => setIsTeamGoalModalOpen(false)} className="absolute top-6 right-6 font-black text-3xl">✕</button>
            <h2 className="text-xl md:text-3xl font-black mb-8 italic uppercase">Team Strategy & Notice</h2>
            <div className="space-y-6 mb-10">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-blue-600 ml-4 uppercase">📢 공지사항 문구</label>
            ...
            <button onClick={async () => {
              ...
              if (!error) { alert("업데이트 완료"); setIsTeamGoalModalOpen(false); fetchTeamGoal(); }
...
function ProgressBar({ label, current, target, unit, color }: any) {
  ...
  return (
    <div className="w-full space-y-2">
      <div className="flex justify-between font-black uppercase text-xs"><span>{label}</span><span>{current}/{target}{unit}</span></div>
      <div className="w-full h-4 bg-slate-100 rounded-full overflow-hidden shadow-inner"><div className="h-full transition-all duration-1000" style={{ width: `${rate}%`, backgroundColor: color }}></div></div>
    </div>
  )
}
    ...
    return (
        <div className="w-full space-y-1">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full" style={{ width: `${rate}%`, backgroundColor: color }}></div>
            </div>
        </div>
    )
}
...
function ActivityTab({ label, value, onChange, color, textColor, borderColor }: any) {
    return (
      <div className={`${color} ${borderColor} border-4 p-3 md:p-6 rounded-[1.5rem] shadow-md active:scale-95 transition-transform`}>
        <label className="text-[8px] md:text-xs font-black block mb-1 uppercase opacity-60">{label}</label>
